...
<div class="row">
	...
	<div class="col-lg-5">
	<h4>DMS Track Courses</h4>
	<p> Course tagged with * are core courses for DMS</p>
		<table class="table table-striped">
			<tr>
				<th>Course ID</th>
				<th>Course Name</th>
			...
			<tr>
			<?php
			$sql = "SELECT * FROM course where requirement = 'program' and (major ='".$MAJOR."' or major ='general') and (track='dms')  order by track_type, code";
			//echo $sql;
			$result = $dbconn->query($sql);
			$total_credit_hours=0;
			while($row = $result->fetch_assoc()){
				...
				$course_name=$row['name'];
				$core="";
				if ($row['track_type']=='core') $core="*";
				echo'<tr><td id="'.$course_code.'" ondblclick=\'editCell("'.$course_code.'")\' oncontextmenu=\'saveCodeCell("'.$course_code.'")\' >'.$row['code'].' '.$core.'</td><td id="name'.$course_code.'" ondblclick=\'editCell("name'.$course_code.'")\' oncontextmenu=\'saveNameCell("'.$course_code.'")\' >'.$row['name'].'</td><td>'.$row['credit_hours'].'</td> </tr> ';
	...
	<h4>Security Track Courses</h4>
		<table class="table table-striped">
			...
			<tr>
			<?php
			$sql = "SELECT * FROM course where requirement = 'program' and (major ='".$MAJOR."' or major ='general') and (track='security')  order by track_type, code";
			//echo $sql;
			$result = $dbconn->query($sql);
			$total_credit_hours=0;
			while($row = $result->fetch_assoc()){
